...
# if object_id  and object_type is not given, it loops through all instances. For object types see list of "somt_" in header_common.py
...
party_set_bandit_attraction            = 1645   # (party_set_bandit_attraction, <party_id>,<attraction>), #set how attractive a target the party is for bandits (0..100)
...
party_get_ignore_with_player_party     = 1649   # (party_get_ignore_with_player_party, <destination>, <party_id>),
...
#for weapons, agent needs to have an empty weapon slot. weapon_slot_no is optional, and can be between 1-4 (used only for weapons, not armor).
...
cur_map_icon_set_tableau_material      = 1983   # (cur_map_icon_set_tableau_material, <tableau_material_id>, <instance_code>), #only call inside ti_on_init_map_icon in module_scene_props; instance_code is simply passed as a parameter to the tableaucur_tableau_add_tableau_mesh           = 1980   # (cur_tableau_add_tableau_mesh, <tableau_material_id>, <value>, <position_register_no>), #value is passed to tableau_material
...
str_store_string                = 2320	# (str_store_string,<string_register>,<string_no>),
...
key_is_down                     = 70  # (key_is_down, <key_code>, [<bypass_console_check>]),
                                      # Checks that the specified key is currently pressed. See header_triggers.py for key code reference.
key_clicked                     = 71  # (key_clicked, <key_code>, [<bypass_console_check>]),
...
game_key_get_key  = 3100 #(game_key_get_key, <destination>, <game_key_no>, [<alternarive>], [<modifier>]), #Stores the key mapped to <game_key_no> into <destination>
...
dict_load_file             = 3202 #(dict_load_file, <dict>, <file>, [<mode>], [<ini>]), #Loads a dictionary file into <dict>. Setting [<mode>] to 0 (default) clears <dict> and then loads the file, setting [<mode>] to 1 doesn't clear <dict> but overrides any key that's already present, [<mode>] to 2 doesn't clear <dict> and doesn't overwrite keys that are already present. Setting [<ini>] to 1 use ini file instead binary
dict_save                  = 3204 #(dict_save, <dict>, <file>, [<ini>]), #Saves <dict> into a file. For security reasons, <file> is just a name, not a full path, and will be stored into a WSE managed directory. Setting [<ini>] to 1 use ini file instead binary
dict_delete_file           = 3209 #(dict_delete_file, <file>, [<ini>]), #Deletes dictionary file <file> from disk. Setting [<ini>] to 1 use ini file instead binary

str_store_module_setting                  = 4226 #(str_store_module_setting, <string_register>, <setting>, <section>), #Stores the string value (empty if not found) of <section>'s <setting> in rgl_config.ini into <string_register>
