...
#Auto-menu
  (0,
   [
	...
	## Wulf/Iron_Sight: edit begin
	## Wulf/Iron_Sight: adding to existing trigger to try to keep savegame compatibility
	(try_begin),
	  (store_item_kind_count, ":load_check", "itm_no_item", "trp_town_walker_1"),
	  (eq, ":load_check", 0), ## loading resets inventory, if 0 &...
	  (eq, "$g_disable_condescending_comments", -2), ## if prejudice is high = game was loaded
	  (call_script, "script_dplmc_init_faction_gender_ratio", 1), ## change female gear to male gear
	  #(display_message, "@Load Check = true"),
	(try_end),
	## Wulf/Iron_Sight: edit end
     ]),
...
#	(assign, ":controversy_deduction", 1),
	...
	(try_for_range, ":active_npc", heroes_begin, heroes_end),
		...
		(neg|main_party_has_troop, ":active_npc"),
		# (neg|troop_slot_eq, ":active_npc", slot_troop_occupation, slto_player_companion), #SB : no controversy decay
	##diplomacy end+
   ...
   [
       ...
       ##diplomacy end+
         ...
         # (else_try), #SB : faction must be active to spawn
           ...
           (try_begin),
             (eq, "$g_there_is_no_available_centers", 0),
...
(12,
   [
   #SB : merged with other 12-hour center loop
   # (try_for_range, ":village", villages_begin, villages_end),
      # (party_set_slot, ":village", dplmc_slot_village_reserved_by_recruiter, 0),
   # (try_end),
   ]),
